...
def guassthird(xvals, yvals, x):
    ...
    for i in range(0,n - 2,2):
        ulist.append(ulist[-1] * (u - num) / (i + 1))
        ulist.append(ulist[-1] * (u + num - 1) / (i + 2))
        num += 1
    mid += 1
    y = yvals[mid]
...
def Bessel(xvals, yvals, x):
    drawtable(table(xvals, yvals))
    print('guass forward:  ')
    gf = guassfor(xvals, yvals, x)
    print('guass Third: ')
    gt = guassthird(xvals, yvals, x)
    y = (gf + gt) / 2
    print('Bessel: \ny = (%f + %f) / 2 = %.6g' % (gf, gt, y))
    return y
print(Bessel(xvals, yvals, x))
